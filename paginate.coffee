@__Pages = class Pages
    ...
    if @infinite
      ...
      if page is 1
        ...
        try
          ...
          false #@ADDED: Catch concat error
      #@redraw()
    else
  ...
  getPage: (page) ->
    ...
    if Meteor.isClient
      if page < @sess "totalPages"
        @recvPages page
        #console.log "getpage(" + page + ") called."
      if @infinite
        unless @cache[1]?
          #console.log "No hay cache"
          return
        #@log @cache[1]
        #console.log "Hay cache: " + @cache[1].length + " elementos."
        tempcache = @uniqueArray @cache[1].slice 0, page * @perPage
  ...
  # Changes session pages when scrolling using infinite, triggering next page
  setInfiniteTrigger: ->
    window.onscroll = (_.throttle ->
      ...
      if t > 1
        ...
        return
      if (window.innerHeight + window.scrollY) >= l and ( @sess("currentPage") < @sess "totalPages" )
        @sess("currentPage", @sess("currentPage") + 1) # @TODO: Remove autoincrement when currentpage is higher than total pages  
  ...
  uniqueArray: (arr) ->
    ...
    while i < len
      id = arr[i]._id
      if idsSeen[id] isnt idSeenValue
        results.push arr[i]
